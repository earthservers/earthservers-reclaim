...
- **Keyboard shortcuts (Space, arrow seek, volume) died after switching
  windows.** Clicks on the video itself land on a native surface outside the
  page, so after any focus change the page never regained the keyboard until
  you clicked some other UI. Clicking the video now restores the keyboard
  along with focusing that pane.
- **Adding a playlist to the queue now autoplays** — it fills every empty
  visible pane with the queued items and starts them.
- **Shrinking the layout (4 → 2 → 1) stops the cut panes** — their videos
  previously kept playing audio invisibly.
...
- **Playlists round out into a full system.** Hover a saved playlist for an
...
- **Delete playlists.** Hover a saved playlist for a ✕ button; clicking it asks
  for confirmation inline in the row (no modal) before deleting.
